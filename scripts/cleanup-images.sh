...
log_info "📦 Policy: Keep $KEEP_COUNT most recent versions"
log_info "🎯 Target: ${SERVICE_NAME}"
echo ""
...
# 해당 서비스의 모든 이미지 (태그별로)
IMAGES=$(docker images "ghcr.io/${GITHUB_USERNAME}/${SERVICE_NAME}" --format "{{.ID}} {{.CreatedAt}}" | sort -k2 -r)

if [ -z "$IMAGES" ]; then
    log_warn "  No images found for ${SERVICE_NAME}"
else
    # 이미지 개수 확인
...
log_info "Kept most recent $KEEP_COUNT versions for ${SERVICE_NAME}"
